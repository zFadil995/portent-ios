...
struct PortentApp: App {
    init() {
        ...
        let syncSecrets = { syncSecretsToLoggingManager() }
        ServiceInstanceStorage.shared(for: .radarr).onChanged = syncSecrets
        ServiceInstanceStorage.shared(for: .sonarr).onChanged = syncSecrets
    }
